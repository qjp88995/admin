...
class News extends Common{
    public function select(){
        if(!request()->has('selectAll')){
            ...
            $start = $limit*($page-1);
            $data = VolunteerNews::where(@$map)->field('content',true)->limit($start, $limit)->order('sort',$sort)->select();
            $count = VolunteerNews::where(@$map)->count();
            return json([
                'count' => $count,
                'data'  => $data
            ]);
        }else{
            $data = VolunteerNews::field('content',true)->order('sort','desc')->select();
            return json($data);
        }
    }
    public function find(){
        $_id = request()->param('_id');
        $data = VolunteerNews::find($_id);
        return json($data);
    }
    ...
    public function insert(){
        ...
        $VolunteerNews = new VolunteerNews;
        ...
        $data['createAt'] = time();
        $data['updateAt'] = time();
        $result = $VolunteerNews->insert($data);
        if ($result) {
    ...
    public function update(){
        ...
        $data['updateAt'] = time();
        $result = $VolunteerNews->update($data);
        if ($result) {
    ...
    public function delete(){
        ...
        $_id = request()->param()['_id'];
        $result = VolunteerNews::destroy($_id);
        if ($result) {
            return json([
                'code' => true,
                'msg'  => '删除成功！'
