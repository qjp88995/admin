...
class Activity extends Common{
    public function select(){
        ...
        $start = $limit*($page-1);
        $data = ActivityActivity::where(@$map)->field('content',true)->limit($start, $limit)->order('time.start','desc')->select();
        $count = ActivityActivity::where(@$map)->count();
        return json([
            'count' => $count,
            'data'  => $data
        ]);
    ...
    public function insert(){
        ...
        $ActivityActivity = new ActivityActivity;
        if(!empty($data['category'])){
            $category = ActivityCategory::field('title,name')->find($data['category']['id']);
            $data['category'] = [
                'id'    => $data['category']['id'],
        ...
        $data['createAt'] = time();
        $data['updateAt'] = time();
        $result = $ActivityActivity->insert($data);
        if ($result) {
    ...
    public function update(){
        ...
        $data['updateAt'] = time();
        $result = $ActivityActivity->update($data);
        if ($result) {
    ...
    public function delete(){
        ...
        $_id = request()->param()['_id'];
        $result = ActivityActivity::destroy($_id);
        if ($result) {
            return json([
                'code' => true,
                'msg'  => '删除成功！'
