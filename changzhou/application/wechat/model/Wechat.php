...
use app\model\database\WechatSetting;
/**
...
<Nonce><![CDATA[%s]]></Nonce>
	...
	protected function setCache($cachename,$value,$expired){
        ...
        $setting = WechatSetting::where('cachename', $cachename)->find();
        if($setting){
            $res = WechatSetting::update([
                '_id'     => $setting->_id,
                'token'   => $value,
                'expired' => time() + $expired
            ]);
        }else{
            $res = WechatSetting::insert([
                'cachename' => $cachename,
                'token'     => $value,
                'expired'   => time() + $expired
            ]);
	...
	protected function removeCache($cachename){
		$setting = WechatSetting::where('cachename', $cachename)->find();
        if($setting){
            WechatSetting::destroy($setting->_id);
        }
		return false;
	}
