...
class Collection extends Auth{
    public function index(){
        ...
        $start = $limit*($page-1);
        $data = CollectionList::where(@$map)->field('content,moreFields',true)->limit($start, $limit)->order('sort',$sort)->select();
    ...
    public function insert(){
        ...
        if(!empty($data['category'])){
            $category = CollectionCate::field('title,name')->find($data['category']['id']['$oid']);
            $data['category'] = [
        ...
        $data['createAt'] = time();
        $data['updateAt'] = time();
        $result = $CollectionList->insert($data);
        if ($result) {
    ...
    public function update(){
        ...
        $data['updateAt'] = time();
        $result = $CollectionList->update($data);
        if ($result) {
    ...
    public function delete(){
        ...
        $_id = request()->param()['_id'];
        $result = CollectionList::destroy($_id);
        if ($result) {
            return json([
                'code' => true,
                'msg'  => '删除成功！'
