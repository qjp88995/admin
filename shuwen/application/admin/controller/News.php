...
class News extends Auth{
    public function index(){
        ...
        $start = $limit*($page-1);
        $data = NewsList::where(@$map)->field('content,moreFields',true)->limit($start, $limit)->order('sort',$sort)->select();
    ...
    public function insert(){
        ...
        if(!empty($data['category'])){
            $category = NewsCate::field('title,name')->find($data['category']['id']['$oid']);
            $data['category'] = [
