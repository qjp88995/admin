...
use app\venue\model\Vcategory as Category;

class Exhibit extends Auth{
    public function index(){
        ...
        $start = $limit*($page-1);
        $data = Exhbt::where(@$map)->field('content,moreFields',true)->limit($start, $limit)->order('sort',$sort)->select();
        $count = Exhbt::where(@$map)->count();
        return json([
            'count' => $count,
            'data'  => $data
        ]);
    ...
    public function insert(){
        ...
        if(!empty($data['category'])){
            $category = Category::field('title,name')->find($data['category']['id']['$oid']);
            $data['category'] = [
